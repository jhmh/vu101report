...
The way researchers share and claim their findings (scholarly communication) is changing under the influence of the application of new communication technologies. In the digital era, the medium has changed, and this influences the way scholars communicate: new ways to *discover* the latest scientific information, setup faster *analyses* designed for reproducibility, *write* collaboratively, *publish* articles and reusable data in new ways on the internet, do the *outreach* of findings in more then one channel, have the *assessment* of findings done in different ways.
...
This project investigates the use of digital tools used in scholarly communication within the VU and VUmc. By pooling this with a global research from Utrecht University by Kramer and Bosman [1], we can compare our results with other countries.
...
The results in this report are meant as a basis for the conversation with the faculties, to improve and evolve library services toto better assist the work of researchers in their activities in scholarly communication.
...
To bring tools for advancing scholarly communication ourselves, we added web annotation to this report using [Hypothes.is](http://Hypothes.is)
...
# Method
To start, we have to realise that the survey design was a given to us. We were not involved from the start, but joined this research project from the Utrecht University when it was well underway. The goal of the researchers Kramer and Bosman was to gain insight in which digital online tools were used during different activities of scholarly communication.
...
1. Tool usage of the VU and VUmc researchers as a monolithic group, with comparisons to other parts of the world.
2. Tool usage of the different disciplines of VU and VUmc, also with comparisons to other parts of the world.

All these questions can be found in this document containing the [VU101 research questions](https://docs.google.com/document/d/1p3k5PLXtJNvGviWvsBoPZ4dbgGU9Fa_dHb08iOdvC6Q/edit?usp=sharing). You can also find here the priority of what questions should be treated first, and the others if time was left for further research.
...
## Data gathering
Next, we needed VU and VUmc researchers to fill in the survey. For this we needed a method to filter these researchers from the data, and we needed to communicate the survey to this specific target group.

For filtering VU and VUmc researchers out of the survey data, we asked the lead researchers to add a hash [7V4u8a] to the survey URL. This custom url can be used in communicating the survey. To monitor the survey activities in real-time ourselves, we created a short URL from the custom URL: [http://bit.ly/vu101innovations] . This url is used in communication to VU and VUmc researchers.

To communicate this survey to this specific target group within the VU and VUmc, we asked the research portfolio managers to distribute this survey in an e-mail from the faculty secretaries. This resulted in 543 visitors in January, and 296 more after a reminder in February, as you can see in the diagram below.
...
From a population of 3772 people academic staff at VU and VUmc; we had in total 839 visits (22%) to this short URL. As you can see in the results section, 531 people (14%) finally filled in the survey.
...
## Data preparation
In April we received the anonymised and cleaned (eg. removed whitespaces) data from the lead researchers. One set with all the data, and one with only respondents from the VU and VUmc.
We prepared the data for making plots by the following procedure; changing columns representing a tool from a string value to a boolean value, next adding a boolean to the tool column where the name of the tool has been written down in the free text field for that respondent. Then we added boolean values to the respondents who are affiliated to an institution in a OECD country. After that we marked which column belongs to what research phase in the scholarly communication cycle (Discovery, Analysis, Writing, Publication, Outreach, Assessment), and within these phases what columns represent what research activity (eg. searching literature, writing collaboratively, sharing data, ...) After that we labeled what tool are supported by the VU library. Extra information from the tool database (eg. tool age, twitter followers and open science category) could be taken into account for making additional correlations, but due to time limitation we left this out of the answers for now.
...
We chose R as the language to make plots, and the report in this HTML file. For each question we have made a plot in a separate R file to ease the procedure of collaboration, code-commenting and debugging. Then we nested this plot in the R-Markdown (.Rmd) format where we could add commenting text like you read here now. All .Rmd files are nested in a master report. This allowed us to make a report that immediately updates the plots when the code changes, keeping the graphs in sync with the latest code and data. In the end it saved us time, and makes the reproducibility of this report as easy as possible.
...
Throughout the results the Scholarly Communication Phases will return in each section as separate paragraphs for Discovery, Analysis, Writing, Publication, Outreach and Assessment.
...
* The demographics section show numbers on segments of the respondents on the survey regarding countries, career groups, and disciplines.
* Popular tools section shows if there is a relation between tools that are supported by the library, and are best known.
* OECD section shows what the differences are between the tool usage of VU and OECD countries.
* Career group section show the differences between tool usage of tenured and non-tenured groups.
...
* Detailed Disciplines section shows the tool usage for each discipline, adding comparison information such as the OECD number for that discipline, and the VU average.
